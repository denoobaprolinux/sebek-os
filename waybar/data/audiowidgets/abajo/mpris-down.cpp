...
public:
  MPRISPopup() {
    ...
    btn_next.signal_clicked().connect(
        sigc::mem_fun(*this, &MPRISPopup::on_next));
...
private:
  ...
  std::string bg_color = "#282828", fg_color = "#ebdbb2",
              accent_color = "#cc241d";

  void load_pywal_colors() {
    const std::string wal_colors_path =
        std::string(std::getenv("HOME")) + "/.cache/wal/colors";
  ...
  void update_info() {
    ...
    label_title.set_markup(std::string("<b>") +
                           (title ? title : "Desconocido") + "</b>");
...
public:
  MPRISApplication() : Gtk::Application("sebekos.mpris.down") {}
};
